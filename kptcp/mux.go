...
func (mux *Mux) Any(urlPattern string, handlers ...func(c *Context)) error {
	if mux.isReadOnly() == false {
		...
		return errors.New(fmt.Sprintf("mux is only writable before mux.LockWrite()"))
	}
}
...
func (mux *Mux) AddHandleFunc(messageID int32, handler func(ctx *Context)) {
	if mux.AllowAdd == false {
		panic(errors.New("mux.AllowAdd is false, you should use AddHandleFunc before it's locked, after calling  ktcp.ListenAndServe(), the mux will be locked"))
	}
	mux.Mutex.Lock()
...
func (mux *Mux) AnchorIndexOfMessageID(messageID int32) int {
	...
	if !ok {
		panic(errors.New(fmt.Sprintf("messageID '%d' anchor not found in mux.MessageIDAnchorMap", messageID)))
	}
...
func (mux *Mux) ReplaceMiddlewareAnchor(anchor MiddlewareAnchor) {
	...
	_, ok := mux.MiddlewareAnchorMap[anchor.MiddlewareKey]
	if !ok {
		panic(errors.New(fmt.Sprintf("mux.MiddlewareAnchorMap['%s'] not exists, can't use ReplaceMiddlewareAnchor", anchor.MiddlewareKey)))
	}
	// replace map
	mux.MiddlewareAnchorMap[anchor.MiddlewareKey] = anchor
...
func (mux *Mux) AddMessageIDAnchor(anchor MessageIDAnchor) {
	...
	if _, ok := mux.MessageIDAnchorMap[anchor.MessageID]; ok {
		panic(errors.New(fmt.Sprintf("mux.MessageIDAnchorMap[%d] already exists", anchor.MessageID)))
	}
...
func (mux *Mux) AddMessageIDSelfMiddleware(messageID int32, handlers ...func(c *Context)) {
	...
	if ok {
		panic(errors.New(fmt.Sprintf("messageIDSelfMiddleware[%d] already exist", messageID)))
	}
...
// this function is designed to exec when ktcp.OnMessage is not nil, on this case mutex routine makes no sense, all registered
