...
func (pm ProtobufMarshaller) Marshal(v interface{}) ([]byte, error) {
	...
	if !ok {
		return nil, errors.New(fmt.Sprintf("protobuf marshaller requires src realize proto.Message"))
	}
...
func (pm ProtobufMarshaller) Unmarshal(data []byte, dest interface{}) error {
	...
	if !ok {
		return errors.New(fmt.Sprintf("protobuf marshaller requires src realize proto.Message"))
	}
