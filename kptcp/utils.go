...
// WriteJSON will write conn a message wrapped by ktcp.JSONMarshaller
...
func PipeJSON(conn net.Conn, args ...interface{}) error {
	...
	if len(args)%2 != 0 {
		return errors.New("iligal args, PipeJSON'args requires messageID int32, data interface{} in pair")
	}
	...
	for i := 0; i < len(args)-1; i += 2 {
		...
		if !ok {
			return errors.New(fmt.Sprintf("wrong type, args[%d] should be a int type messageID but got %s", i, reflect.TypeOf(args[i]).Name()))
		}
		message := args[i+1]
