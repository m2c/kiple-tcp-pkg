...
type Context struct {
	...
	// saves a pool-ref from ktcp instance
	// only when ktcp instance has set builtInPool true, poolRef is not nil
	// - How to use this?
	// `ctx.Online(username)`
	// `ctx.Offline()`
	poolRef *ClientPool
...
// Only used when ktcp instance's builtInPool is true,
// otherwise you should design your own client pool(github.com/fwhezfwhez/ktcp/clientPool/client-pool.go), and manage it
// yourself, like:
// ```
//     var myPool = clientPool.NewClientPool()
//     func main() {
//         srv := ktcp.Newktcp(nil)
//         srv.AddHandler(1, func(c *ktcp.Context){
//             type Login struct{
//                Username string
//             }
//             var userLogin Login
...
//         })
//         srv.AddHandler(2, func(c *ktcp.Context){
...
func (ctx *Context) Online(username string) error {
	...
	if ctx.poolRef == nil {
		return errors.New("ctx.poolRef is nil, did you call 'ktcp.WithBuiltInPool(true)' or 'ktcp.SetPool(pool *ktcp.ClientPool)' yet")
	}
...
// Key should not start with 'ktcp-', or it will panic.
func (ctx *Context) SetCtxPerConn(k, v interface{}) {
	if tmp, ok := k.(string); ok {
		if strings.HasPrefix(tmp, "ktcp-") {
			panic("keys starting with 'ktcp-' are not allowed setting, they're used officially inside")
		}
	}
...
func (ctx *Context) Username() (string, bool) {
	usernameI, ok := ctx.GetCtxPerConn("ktcp-username")
	if !ok {
		return "", ok
	}
...
func (ctx *Context) SetUsername(username string) {
	ctx.setCtxPerConn("ktcp-username", username)
}
...
func (ctx *Context) HeartBeatChan() chan int {
	channel, ok := ctx.GetCtxPerConn("ktcp-heart-beat-channel")
	if !ok {
		channel = make(chan int, 1)
		ctx.setCtxPerConn("ktcp-heart-beat-channel", channel)
